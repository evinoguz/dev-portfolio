    ...
    >
      <div className="w-full h-full p-4 bg-white rounded-lg dark:bg-gray-900">
        <div className="flex flex-col items-center space-y-4 ">
          <img src={image.url} alt={title} className="object-cover w-full rounded-lg min-h-40 max-h-40" />
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {tags.map((tag, key) => (
              <span
                key={key}
                className="flex items-center justify-center px-2 py-1 my-1 text-xs font-bold leading-none text-pink-100 bg-pink-700 rounded"
              >
                {tag}
              </span>
            ))}
